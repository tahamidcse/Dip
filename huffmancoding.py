...
def main():
    ...
    with rawpy.imread('20251028_111957.dng') as raw:
         rgb = raw.postprocess() 
    # ...
    img =  cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY)
        # Load grayscale image (CHANGE PATH)
    img2=cv2.imread('20251028_111957.jpg',0)
    ...
    freq_map2 = calculate_frequencies(img2)  # <-- freq_map is calculated here
    ...
    huffman_table_list2 = [[pixel, code] for pixel, code in sorted(huffman_codes2.items())]
...
# Convert to 2D NumPy array of object type (since codes are strings)
    ...
    huffman_table_matrix2 = np.array(huffman_table_list2, dtype=object)
# -------------------------------
